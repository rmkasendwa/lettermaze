...
export default function GameResultPage() {
  ...
  if (loading)
    return (
      <main className="mx-auto max-w-3xl px-4 py-12">Loading result…</main>
    );
  if (!user)
    return (
      <main className="mx-auto max-w-3xl px-4 py-12">
        Log in to inspect saved games.
      </main>
    );
  if (failed)
    return (
      <main className="mx-auto max-w-3xl px-4 py-12">
        This game result could not be found.
      </main>
    );
  if (!game)
    return (
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-8 sm:py-12">
      ...
      <header className="mt-6 rounded-2xl bg-gradient-to-br from-violet-700 to-indigo-700 p-6 text-white shadow-xl sm:p-8">
        ...
        <h1 className="mt-2 text-4xl font-black tabular-nums">
          {game.score} points
        </h1>
        <time
          className="mt-2 block text-violet-100"
      ...
      <dl className="mt-6 grid grid-cols-3 gap-3">
        <ResultStat
          label="Board"
          value={`${game.boardSize}×${game.boardSize}`}
        />
        <ResultStat
          label="Duration"
          value={`${Math.floor(game.durationSeconds / 60)}:${String(game.durationSeconds % 60).padStart(2, "0")}`}
        />
        <ResultStat label="Words" value={game.wordsFound} />
      </dl>
      <section className="mt-8" aria-labelledby="words-heading">
        <h2 className="text-2xl font-bold" id="words-heading">
          Words found
        </h2>
        {game.words.length ? (
          <ul className="mt-4 flex flex-wrap gap-2">
            {game.words.map((word) => (
              <li
                className="rounded-lg bg-emerald-100 px-3 py-2 font-semibold uppercase text-emerald-900 dark:bg-emerald-950 dark:text-emerald-100"
                key={word}
              >
                {word}
              </li>
