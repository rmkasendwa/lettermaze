-- `User` was present in the Prisma schema before migrations were introduced,
-- but the original migration history never created it. Create it when this is
-- a fresh database while remaining compatible with databases that already
-- have the table (for example, databases previously initialized by db push).
